    ...
    loadData (pagination, filters, sorter) {
      ...
      if ((typeof result === 'object' || typeof result === 'function') && typeof result.then === 'function') {
        result.then(r => {
          ...
          if (r.length === 0 && _this.showPagination && _this.localPagination.current > 1) {
            _this.localPagination.current--
            _this.loadData()
            return
          }

          // 这里用于判断接口是否有返回 r.totalCount 且 _this.showPagination = true 且 pageNo 和 pageSize 存在 且 totalCount 小于等于 pageNo * pageSize 的大小
